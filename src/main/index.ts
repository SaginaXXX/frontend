...
// Fallback to software rendering on systems without proper GPU/VA-API
...
function setupIPC(): void {
  ...
  ipcMain.on("window-toggle-fullscreen", () => {
    ...
    if (window) {
      ...
      console.log(`Current fullscreen state: ${isFullScreen}, toggling to: ${!isFullScreen}`);
      window.setFullScreen(!isFullScreen);
    } else {
      console.warn('No window available for fullscreen toggle');
    }
  });
...
app.whenReady().then(() => {
  electronApp.setAppUserModelId("com.aiprojectx.app");
