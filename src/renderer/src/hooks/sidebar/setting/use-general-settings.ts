...
export const useGeneralSettings = ({
  ...
  const { showSubtitle, setShowSubtitle } = useChatStore();
  ...
  useEffect(() => {
    ...
    if (newBgUrl && bgUrlContext) {
      ...
      if (bgUrlContext.backgroundUrl !== fullUrl) {
        bgUrlContext.setBackgroundUrl(fullUrl);
      }
    }

    if (settings.wsUrl && settings.wsUrl !== wsUrl) {
      onWsUrlChange(settings.wsUrl);
    }
    if (settings.baseUrl && settings.baseUrl !== baseUrl) {
      onBaseUrlChange(settings.baseUrl);
    }
  }, [settings, wsUrl, baseUrl]);
