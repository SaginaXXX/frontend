...
interface UseDraggableProps {
  ...
  componentId: string
  /**
   * Base transform string applied before drag offsets.
   * Example: 'translateX(-50%)' to preserve horizontal centering while dragging
   */
 ...
 */
export function useDraggable({ isPet = false, componentId, baseTransform = '' }: UseDraggableProps) {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      elementRef.current.style.transform = `${base}translate(${positionRef.current.x}px, ${positionRef.current.y}px)`;
    };
    ...
    const cleanupOnUnload = () => {
      try { document.removeEventListener('mousemove', handleMouseMove, true); } catch {}
      try { document.removeEventListener('mouseup', handleMouseUp, true); } catch {}
    };
    window.addEventListener('beforeunload', cleanupOnUnload, { once: true });
    // Remove this helper once mouseup occurs
